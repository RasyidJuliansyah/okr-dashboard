...
const defaultObjectives = [
    { id: "obj-1", title: "Meningkatkan Profitabilitas Perusahaan", team: "finance", perspective: "financial" },
    { id: "obj-2", title: "Meningkatkan Kepuasan dan Loyalitas Pelanggan", team: "growth", perspective: "customer" },
    { id: "obj-3", title: "Optimalisasi Proses Delivery Fitur Produk", team: "product", perspective: "internal-process" },
    { id: "obj-4", title: "Meningkatkan Kapabilitas dan Kesejahteraan Tim", team: "engineering", perspective: "learning-growth" }
];

const defaultKRs = [
    { id: "kr-1-1", objId: "obj-1", title: "Mencapai Revenue Semester I sebesar $2.0M", owner: "Jane Doe", team: "finance", current: 1.6, target: 2.0, unit: "M USD", status: "on-track", direction: "up", source: "Manual Input", lastUpdated: "2026-07-03 08:30" },
    { id: "kr-1-2", objId: "obj-1", title: "Mengurangi Operating Expenses sebesar 10%", owner: "Jane Doe", team: "finance", current: 4, target: 10, unit: "%", status: "off-track", direction: "up", source: "Manual Input", lastUpdated: "2026-07-03 09:00" },
    { id: "kr-2-1", objId: "obj-2", title: "Meningkatkan Net Promoter Score (NPS) ke 75", owner: "Sarah Smith", team: "growth", current: 70, target: 75, unit: "Poin", status: "on-track", direction: "up", source: "Manual Input", lastUpdated: "2026-07-02 14:15" },
    { id: "kr-2-2", objId: "obj-2", title: "Menurunkan Customer Churn Rate di bawah 2%", owner: "Sarah Smith", team: "growth", current: 3.5, target: 2.0, unit: "%", status: "at-risk", direction: "down", source: "Manual Input", lastUpdated: "2026-07-03 07:45" },
    { id: "kr-3-1", objId: "obj-3", title: "Mempercepat Cycle Time deployment menjadi 3 hari", owner: "Alex Rivera", team: "engineering", current: 5, target: 3, unit: "Hari", status: "at-risk", direction: "down", source: "Manual Input", lastUpdated: "2026-07-03 10:10" },
    { id: "kr-3-2", objId: "obj-3", title: "Meningkatkan test coverage kode utama ke 90%", owner: "Alex Rivera", team: "engineering", current: 88, target: 90, unit: "%", status: "on-track", direction: "up", source: "Manual Input", lastUpdated: "2026-07-02 16:30" },
    { id: "kr-4-1", objId: "obj-4", title: "Menyelesaikan sertifikasi cloud untuk 15 engineer", owner: "Bob Johnson", team: "engineering", current: 12, target: 15, unit: "Orang", status: "on-track", direction: "up", source: "Manual Input", lastUpdated: "2026-07-03 11:20" },
    { id: "kr-4-2", objId: "obj-4", title: "Meningkatkan Employee Engagement Score ke 4.5/5", owner: "HR Team", team: "engineering", current: 4.2, target: 4.5, unit: "Skor", status: "on-track", direction: "up", source: "Manual Input", lastUpdated: "2026-07-01 10:00" }
];
...
const defaultAuditLog = [
    { timestamp: "2026-07-03 11:20", user: "Admin", action: "Update Capaian", target: "Menyelesaikan sertifikasi cloud untuk 15 engineer", oldVal: "10", newVal: "12" },
    { timestamp: "2026-07-03 10:10", user: "Admin", action: "Update Capaian", target: "Mempercepat Cycle Time deployment menjadi 3 hari", oldVal: "6", newVal: "5" },
    { timestamp: "2026-07-03 09:00", user: "Admin", action: "Update Capaian", target: "Mengurangi Operating Expenses sebesar 10%", oldVal: "2", newVal: "4" }
];

const defaultNotifications = [
    { id: "n1", type: "warning", message: "Key Result 'Mempercepat Cycle Time deployment' berada pada status At Risk (5/3 Hari).", time: "2 jam lalu", read: false },
    { id: "n2", type: "danger", message: "Key Result 'Mengurangi Operating Expenses' berada pada status Off Track (4/10%).", time: "4 jam lalu", read: false }
];

// Initialize State
let state = {
    objectives: JSON.parse(localStorage.getItem("dashboard_objectives")) || defaultObjectives,
    keyResults: JSON.parse(localStorage.getItem("dashboard_krs")) || defaultKRs,
    causalRelations: JSON.parse(localStorage.getItem("dashboard_causal")) || defaultCausalRelations,
    auditLog: JSON.parse(localStorage.getItem("dashboard_audit")) || defaultAuditLog,
    notifications: JSON.parse(localStorage.getItem("dashboard_notifications")) || defaultNotifications,
    currentRole: "c-level",
...
const roleInfoMap = {
    "c-level": { name: "C-Level Executive", label: "Company-wide Strategy View", avatar: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=100&q=80" },
    ...
    "admin": { name: "Sistem Admin", label: "Admin Console Active", avatar: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&w=100&q=80" }
};
...
function renderDashboard() {
    ...
    state.objectives.forEach(obj => {
        ...
        const isManager = state.currentRole === "manager" && (obj.team === "product" || obj.team === "engineering");
        const isEmployee = state.currentRole === "employee" && (obj.team === "engineering");
        ...
        if (objKRs.length === 0 && (filterTeam.value !== "all" || filterStatus.value !== "all" || searchInput.value !== "")) {
...
function filterAndSearchKRs() {
    ...
    return state.keyResults.filter(kr => {
        ...
        const matchesSearch = kr.title.toLowerCase().includes(searchVal) || kr.owner.toLowerCase().includes(searchVal);
...
function renderBSCGrid() {
    ...
    perspectives.forEach(p => {
        ...
        const avgBscProgress = pKRs.length > 0 ? Math.round(pKRs.reduce((acc, kr) => acc + calculateKRProgress(kr), 0) / pKRs.length) : 0;
        ...
        if (pKRs.length > 3) {
            ...
            moreDiv.style.cssText = "font-size: 0.75rem; color: var(--primary); text-align: right; padding-top: 4px; font-weight: 500;";
...
if (overrideForm) {
    overrideForm.addEventListener("submit", (e) => {
        ...
        let status = "on-track";
        const progress = kr.direction === "down" 
            ? (newVal <= kr.target ? 100 : Math.round((kr.target / newVal) * 100))
            : (newVal >= kr.target ? 100 : Math.round((newVal / kr.target) * 100));
        ...
        state.keyResults[krIndex].lastUpdated = new Date().toISOString().slice(0, 16).replace("T", " ");
        ...
        alert("Pencapaian Key Result berhasil diperbarui dan dicatat dalam audit trail!");
    });
}
