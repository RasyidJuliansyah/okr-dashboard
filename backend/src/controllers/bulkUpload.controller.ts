...
function normalizeBscPerspective(val: string): string {
  if (!val) return 'FINANCIAL';
  const clean = val.toUpperCase().trim().replace(/[-\s]+/g, '_');
  if (clean.includes('FINANC')) return 'FINANCIAL';
  if (clean.includes('CUSTOM')) return 'CUSTOMER';
  if (clean.includes('PROCESS') || clean.includes('INTERNAL')) return 'INTERNAL_PROCESS';
  if (clean.includes('LEARN') || clean.includes('GROWTH')) return 'LEARNING_GROWTH';
  if (VALID_BSC_PERSPECTIVES.includes(clean)) return clean;
  return 'FINANCIAL';
}
 ...
 * }
 */
export async function bulkUploadKRs(req: AuthRequest, res: Response) {
  try {
    ...
    for (let index = 0; index < keyResults.length; index++) {
      ...
      try {
        ...
        if (row.departments) {
          const deptNames = String(row.departments).split(/[,;]+/).map((d: string) => d.trim()).filter(Boolean);
 ...
 * }
 */
export async function bulkUploadInitiatives(req: AuthRequest, res: Response) {
  try {
    ...
    const krLookup = new Map<string, string>();
