...
exports.assignUsersToKeyResult = assignUsersToKeyResult;
exports.getKeyResultAssignments = getKeyResultAssignments;
exports.getMyAssignedKrs = getMyAssignedKrs;
exports.getKrsForInitiativeDropdown = getKrsForInitiativeDropdown;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
const VALID_BSC_PERSPECTIVES = ['FINANCIAL', 'CUSTOMER', 'INTERNAL_PROCESS', 'LEARNING_GROWTH'];
async function createKeyResult(req, res) {
    try {
...
async function deleteKeyResult(req, res) {
    try {
        const { id } = req.params;
        // Check if KR exists
...
async function updateKeyResultProgress(req, res) {
    try {
        const { id } = req.params;
...
async function getKeyResultHistory(req, res) {
    try {
        const { id } = req.params;
...
async function assignUsersToKeyResult(req, res) {
    try {
        const { id } = req.params;
        ...
        if (Array.isArray(req.body.userIds)) {
            normalizedAssignments = req.body.userIds.map((uid) => ({
                userId: uid,
...
async function getKeyResultAssignments(req, res) {
    try {
        const { id } = req.params;
        const kr = await prisma.keyResult.findUnique({
            where: { id },
...
async function getMyAssignedKrs(req, res) {
    try {
...
async function getKrsForInitiativeDropdown(req, res) {
    try {
