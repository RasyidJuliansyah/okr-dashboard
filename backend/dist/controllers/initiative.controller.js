...
exports.updateInitiativeKanbanStatus = updateInitiativeKanbanStatus;
exports.deleteInitiative = deleteInitiative;
exports.getMyWork = getMyWork;
exports.getMyTeamInitiatives = getMyTeamInitiatives;
exports.getKpisForInitiative = getKpisForInitiative;
exports.createKpi = createKpi;
exports.updateKpi = updateKpi;
exports.deleteKpi = deleteKpi;
exports.assignUsersToKpi = assignUsersToKpi;
exports.submitKpiUpdate = submitKpiUpdate;
exports.getKpiUpdates = getKpiUpdates;
exports.approveKpiUpdate = approveKpiUpdate;
exports.rejectKpiUpdate = rejectKpiUpdate;
exports.getPendingKpiUpdates = getPendingKpiUpdates;
...
async function getInitiatives(req, res) {
    try {
        ...
        // 2. LEADER (P): melihat card miliknya sendiri + semua card anggota tim di bawahnya
            ...
            const teamIdSet = new Set(leaderTeams.map(t => t.id));
            if (dbUser?.teamId)
                teamIdSet.add(dbUser.teamId);
        ...
        // 3. MANAGER (M): melihat semua card P (Leader) dan T (Team) di departemen yang dikelola
            ...
            const deptValues = new Set(managedDepts.map(d => d.value));
            ...
            const teamIdSet = new Set(deptTeams.map(t => t.id));
            if (dbUser?.teamId)
                teamIdSet.add(dbUser.teamId);
            const teamIds = Array.from(teamIdSet);
...
async function createInitiative(req, res) {
    try {
        ...
        if (!teamId) {
            ...
            if (!firstTeam)
                return res.status(400).json({ message: 'Belum ada tim terdaftar di sistem' });
            teamId = firstTeam.id;
        }
...
async function updateInitiativeKanbanStatus(req, res) {
    try {
        const { id } = req.params;
...
async function getMyTeamInitiatives(req, res) {
    try {
...
async function getKpisForInitiative(req, res) {
    try {
...
async function assignUsersToKpi(req, res) {
    try {
        const { id } = req.params;
...
async function submitKpiUpdate(req, res) {
    try {
...
async function getKpiUpdates(req, res) {
    try {
        const { id: kpiId } = req.params;
...
async function approveKpiUpdate(req, res) {
    try {
...
async function rejectKpiUpdate(req, res) {
    try {
        const { updateId } = req.params;
...
async function getPendingKpiUpdates(req, res) {
    try {
        const updates = await prisma.kpiUpdate.findMany({
