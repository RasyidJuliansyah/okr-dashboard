...
exports.getCLevelBscDashboard = getCLevelBscDashboard;
...
async function getCLevelBscDashboard(req, res) {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        keyResults.forEach(kr => {
            ...
            if (kr.status === 'ON_TRACK')
                perspectiveMap[p].onTrackCount++;
            else if (kr.status === 'AT_RISK')
                perspectiveMap[p].atRiskCount++;
            else if (kr.status === 'OFF_TRACK')
                perspectiveMap[p].offTrackCount++;
        ...
        const trendData = {};
        PERSPECTIVES.forEach(p => {
            trendData[p] = trendAccum[p].map(b => b.count > 0 ? Math.round((b.sum / b.count) * 10) / 10 : null);
        });
